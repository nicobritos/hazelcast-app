...
public class NeighbourMaxStreetReducerFactoryTest {
    ...
    public void getTotalQtyOfTreesByStreetOfEachNeighbourhood() throws ExecutionException, InterruptedException {
        ...
        JobCompletableFuture<Map<String, MapEntrySimple<String, Long>>> result = job.mapper(new StreetCountMapper())
                .combiner(new StreetCountCombinerFactory())
                .reducer(new NeighbourMaxStreetReducerFactory()).submit();
        Map<String, MapEntrySimple<String, Long>> ans = result.get();
        Assert.assertEquals(ans.get("n1").getKey(),"st1");
        Assert.assertEquals(ans.get("n1").getValue().intValue(),4);
        Assert.assertEquals(ans.get("n2").getKey(),"st1");
        Assert.assertEquals(ans.get("n2").getValue().intValue(),2);
        Assert.assertEquals(ans.get("n3").getKey(),"st1");
        Assert.assertEquals(ans.get("n3").getValue().intValue(),2);
        Assert.assertEquals(ans.get("n4").getKey(),"st1");
        Assert.assertEquals(ans.get("n4").getValue().intValue(),1);
        Assert.assertEquals(ans.get("n5").getKey(),"st1");
        Assert.assertEquals(ans.get("n5").getValue().intValue(),1);
