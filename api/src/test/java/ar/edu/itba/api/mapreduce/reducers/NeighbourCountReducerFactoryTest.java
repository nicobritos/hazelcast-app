...
public class NeighbourCountReducerFactoryTest {
    ...
    public void getTotalTreesQtyByNeighbour() throws ExecutionException, InterruptedException {
        ...
        JobCompletableFuture<Map<String, Long>> result = job.mapper(new NeighbourCountMapper())
                .combiner(new StringCountCombinerFactory())
                .reducer(new NeighbourCountReducerFactory()).submit();
        Map<String,Long> ans = result.get();
        Assert.assertEquals(ans.get("n1").intValue(), 4);
        Assert.assertEquals(ans.get("n2").intValue(), 2);
        Assert.assertEquals(ans.get("n3").intValue(), 2);
        Assert.assertEquals(ans.get("n4").intValue(), 1);
        Assert.assertEquals(ans.get("n5").intValue(), 1);
    }
}
